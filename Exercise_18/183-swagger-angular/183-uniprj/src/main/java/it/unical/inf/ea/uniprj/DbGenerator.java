...
import it.unical.inf.ea.uniprj.data.service.UserService;
...
public class DbGenerator implements ApplicationRunner {
  ...
  protected final CourseMaterialService courseMaterialService;
  ...
  public void createDb() {
    ...
    try {
      ...
      for (CSVRecord record : usersCsv) {
        insertUser(record.get(0), record.get(1), record.get(2));
      }
    } catch (IOException e) {
  ...
  private void insertUser(String email, String password, String roles) {
    User user = new User();
    user.setEmail(email);
