...
public interface AccountDao extends JpaRepository<Account, Long> {
  ...
  Optional<Account> getBy(@Param("cred") Credential credential);

  @Query("from Account a where a.credential.username=:username and a.credential.password=:passw")
  Optional<Account> getBy(@Param("username") String usern, @Param("passw")String pass);

  Optional<Account> findByCredential(Credential credential);

  Optional<Account> findByCredentialUsernameAndCredentialPassword(String username, String password);
